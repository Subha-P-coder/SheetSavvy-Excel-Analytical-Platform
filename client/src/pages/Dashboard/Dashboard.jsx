    ...
    <div className="dashboard-wrapper">
      ...
      <div className="main-content">
        ...
        <div className="page-content">
          <h1>{userData?.name ? `Welcome ${userData.name}!` : 'Welcome to the Dashboard!'}</h1>
        ...
        <div className="dashboard-stats-wrapper">
          <div className="stat-card">
            ...
            <h3>Total Files</h3>
            <p><CountUp end={stats.totalFiles} duration={1.5} /></p>
          </div>

          <div className="stat-card">
            <FaChartBar size={32} className="stat-icon" />
            <h3>Charts Created</h3>
            <p><CountUp end={stats.chartsCreated} duration={1.5} /></p>
          </div>

          <div className="stat-card">
            <FaRobot size={32} className="stat-icon" />
            <h3>AI Insights</h3>
            <p><CountUp end={stats.aiInsights} duration={1.5} /></p>
          </div>

          <div className="stat-card">
            <FaDatabase size={32} className="stat-icon" />
            <h3>Total File Size</h3>
            <p><CountUp end={Math.round(stats.totalFileSize / 1024)} duration={1.5} /> KB</p>
          </div>
        </div>
              ...
              recentCharts.map((chart, idx) => (
                <div className="recent-card" key={idx}>
                  ...
                  {chart.config?.xField && chart.config?.yField && (
                    <p>X: {chart.config.xField}, Y: {chart.config.yField}</p>
                  )}
                </div>
              ))
            )}
