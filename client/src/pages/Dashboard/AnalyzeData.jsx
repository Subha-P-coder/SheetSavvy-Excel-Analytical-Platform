...
const AnalyzeData = () => {
  ...
  const chartTypes = ['bar', 'line', 'pie', 'doughnut', 'radar', 'polar', 'scatter', 'bubble', 'scatter3d', 'surface', 'bar3d'];

  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await axios.get(`${backendUrl}/api/excel/record/${recordId}`, {
        ...
        if (data.length > 0) {
          ...
          const numericKeys = keys.filter(key => !isNaN(data[0][key]) && isFinite(data[0][key]));
          ...
          {fetchedData.length > 0 && (
            <>
              <div className="axis-selectors">
                ...
                {(chartType === 'scatter3d' || chartType === 'surface' || chartType === 'bar3d') && (
                  <label>
                    Z-Axis:
              ...
              <div className="button-group">
                <button className="chart-btn" onClick={exportAsPDF}>Export as PDF</button>
                <button className="chart-btn" onClick={downloadAsImage}>Download Image</button>
              </div>
            </>
          )}
        </div>
