...
router.get('/analyze/:id', auth, async (req, res) => {
  try {
    ...
    if (!record || !record.data || record.data.length === 0)
      return res.status(404).json({ success: false, message: 'No data found for analysis' });
...
router.get('/plotly/:id', auth, async (req, res) => {
  try {
    const record = await ExcelData.findById(req.params.id);
    if (!record || !record.data || record.data.length === 0)
      return res.status(404).json({ success: false, message: 'No data found for 3D plot' });

    const data = record.data;
    const numericKeys = Object.keys(data[0]).filter(key => typeof data[0][key] === 'number');

    if (numericKeys.length < 3)
      return res.status(400).json({ success: false, message: 'Need at least 3 numeric fields for 3D plot' });
    ...
    res.status(500).json({ success: false, message: 'Failed to process 3D plot data' });
  }
});
...
router.get('/history', auth, async (req, res) => {
  try {
    const records = await ExcelData.find({ uploadedBy: req.user.id }).sort({ uploadedAt: -1 });
...
router.post('/recent-chart-insight', auth, async (req, res) => {
  try {
    const { type, xField, yField, zField, rField, config = {}, fileId } = req.body;
